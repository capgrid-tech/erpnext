...
from frappe.utils import cint, formatdate, get_first_day, get_last_day, get_link_to_form, get_timestamp, today
...
def update_company_current_month_sales(company):
	# Calculate first and last day of current month for efficient indexing
	...
	results = frappe.db.sql(
		...
		WHERE
			...
			AND company = %s
	""",
		(current_month_start, current_month_end, company),
		as_dict=True,
	)

	monthly_total = results[0]["total"] if len(results) > 0 and results[0]["total"] else 0
